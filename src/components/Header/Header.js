...
const Header = (props) => {
  ...
  const valueDateInputShow = `${selectionRange.startDate.toLocaleDateString('en-GB', options)} to ${selectionRange.endDate.toLocaleDateString('en-GB', options)}`
  ...
  const handleSelect = (ranges) => {
    ...
    setInputDate(`${ranges.selection.startDate.toLocaleDateString('en-GB', options)} to ${ranges.selection.endDate.toLocaleDateString('en-GB', options)}`);
  }
    ...
    <div className='header_component'>
      ...
      <div className='search_area'>
        <div>
          ...
          <input className='search_keyword' placeholder='Where are you going ?'></input>
        </div>
        <div className='date_input_area'>
          <FontAwesomeIcon icon="fa-solid fa-calendar-days" />
          {/* Input for date range, click to show/ hide DateRange*/}
          <input className='date_input' onClick={handleButtonClick} placeholder={inputDate} readonly></input>
        ...
        <Link className='btn_search' to="/search">Search</Link>
      </div>
    </div>
  );
};
